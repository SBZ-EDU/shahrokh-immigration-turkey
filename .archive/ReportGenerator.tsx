...
// FIX: Changed SavedReport to SavedApplication to match the current type definitions.
import { SavedApplication, useLanguage, Page } from '../types';

interface MyReportsPageProps {
  // FIX: Updated prop name and type.
...
// FIX: Updated prop name in function signature.
const MyReportsPage: React.FC<MyReportsPageProps> = ({ savedApplications, onRestore, onDelete, setPage }) => {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 animate-fade-in">
      ...
      <div className="mt-16 max-w-4xl mx-auto">
        {/* FIX: Updated state variable. */}
        {savedApplications.length > 0 ? (
          <div className="space-y-6">
            {/* FIX: Updated state variable. */}
            {savedApplications.map(plan => (
              <div key={plan.id} className="bg-gray-800/50 border border-white/10 rounded-lg shadow-md p-4">
                <div className="flex justify-between items-center flex-wrap gap-2">
                   ...
                   <div className="flex-shrink-0 flex items-center space-x-2">
                     <button onClick={() => onRestore(plan.id)} title={t('myApplications.restore')} className="p-2 text-gray-400 rounded-full hover:bg-gray-700 hover:text-blue-400 transition-all transform hover:scale-110">
                     ...
                     <button onClick={() => onDelete(plan.id)} title={t('myApplications.delete')} className="p-2 text-gray-400 rounded-full hover:bg-gray-700 hover:text-red-400 transition-all transform hover:scale-110">
                ...
                <div className="mt-2 text-sm text-gray-400">
                  <time dateTime={new Date(plan.timestamp).toISOString()}>
                    {t('myApplications.savedOn')} {new Date(plan.timestamp).toLocaleString(language === 'fa' ? 'fa-IR' : 'en-US')}
                  </time>
                </div>
              </div>
            ))}
          ...
          <div className="text-center bg-gray-800/50 border border-dashed border-gray-600 rounded-lg p-12">
            ...
            <div className="mt-6">
              <button
                type="button"
                // FIX: Corrected page navigation to a valid page.
